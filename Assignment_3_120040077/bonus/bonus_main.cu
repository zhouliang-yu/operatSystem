...
#define LOCK(); for(int p=0;p<4;p++){if(threadIdx.x==p){
#define UNLOCK(); }__syncthreads();}
...
__global__ void mykernel(int input_size) {
 ...
 /** calling the __synthreads() instrincfunction to specify the synchronization points in the kernel
  * __synthreads acts as barrier at which threads in the block must wait before any is allowed to proceed*/
  LOCK();
